...
const PostCard: React.FC<{ post: LinkedInPost, index: number }> = ({ post, index }) => {
    ...
    return (
        <div 
            className="bg-card border border-gray-700/50 rounded-lg p-6 animate-card-enter opacity-0"
        ...
        >
            <div className="flex items-center mb-4">
                <img 
                    ...
                    className="w-12 h-12 rounded-full mr-4 border-2 border-card"
                />
                <div>
                    <div className="flex items-center gap-1">
                        <h4 className="font-bold text-light">Muhammad Ahsan Khan</h4>
                        <VerifiedIcon />
                    </div>
                    <p className="text-sm text-gray-400">@mak_web3 · {post.timestamp}</p>
                </div>
            </div>
            <div 
                className="text-gray-300 mb-6 prose-post"
...
const LinkedInFeed: React.FC<LinkedInFeedProps> = ({ data, posts }) => {
    return (
        <section className="py-16 md:py-24 border-t border-card/50 mt-16">
            <div className="container mx-auto px-4">
                ...
                <div className="max-w-2xl mx-auto space-y-6">
                 ...
                 <div className="text-center mt-12">
                     <a href="#" target="_blank" rel="noopener noreferrer" className="bg-card hover:bg-gray-700/50 text-light font-semibold py-3 px-8 rounded-lg shadow-lg transition-transform transform hover:scale-105 duration-300">
                        {data.cta}
                    </a>
                </div>
            </div>
